...
- [x] **B2. Transisi maintenance bebas.** `PATCH /maintenance-requests/:id/status`
...
- [x] **B3. Race assignment ganda.** Cek `asset.status !== available` di luar
      ...
      Fix: `SELECT ... FOR UPDATE` + cek ulang dalam transaksi.
- [x] **B4. Audit `changed_by` salah orang.** `createMaintenance`
      ...
      Fix: controller teruskan `req.user.id` sebagai `changed_by`.
- [x] **B5. IDOR assignment + maintenance detail.** `GET /asset-assignments/:id`
      dan `GET /maintenance-requests/:id/detail` tanpa `roleMiddleware`.
      Repro: employee A `GET` data milik employee B → 200.
      Fix: tetap terbuka untuk semua role login, controller cek kepemilikan
      (`employee.id === requested_by/employee_id`), selain itu 403.
- [x] **B6. Crash 500 ganti 404.** `PATCH /maintenance-requests/:id/status`
      ...
      Fix: guard + `statusCode` + errorMiddleware teruskan status eksplisit.
- [x] **B7. Return timpa maintenance.** Cek `under_maintenance` di luar transaksi,
      ...
      Fix: cek ulang dalam transaksi di bawah lock.
- [x] **B8. Route frontend tanpa guard role.** `App.jsx` hanya cek token.
      Repro: employee buka `/employees`, `/audit-logs`, `/assets/new` → render + fetch 403 sia-sia.
      Fix: `ProtectedRoute` terima prop `roles`, route staff-only diguard.
- [x] **B9. Manager read-only (KEPUTUSAN: ikut PRD 4.3 + README).**
      ...
      Fix: cabut `manager` dari kedua route (frontend memang sudah sembunyi).
- [x] **B10. Validasi form vs backend tidak sinkron.** Form employee hanya wajibkan
...
- [x] **B11. Status employee tak di-whitelist.** `PUT /employees/:id` teruskan
      string apa pun ke `ENUM(active,inactive)` → 500, seharusnya 400.
- [x] **B12. Paginasi tak divalidasi.** `page/limit` mentah ke `LIMIT/OFFSET`.
      Repro: `?limit=abc` / `limit=0` → 500 atau `total_pages: NaN`.
      Fix: clamp page ≥ 1, limit 1–100, `"all"` dipertahankan.
- [x] **B13. Complete buta set `assigned`.** Maintenance aset `available`
      (tanpa assignment) → complete → jadi `assigned` tanpa baris assignment.
      (ikut Batch 1, jalur transisi yang sama; kini hanya dari `under_maintenance` → `assigned`.)
- [x] **B14. Deactivate ber-assignment.** `DELETE /employees/:id` tanpa cek
      assignment aktif → aset terjebak `assigned`, histori yatim.
      Fix: tolak 409 bila masih ada assignment aktif.
- [x] **B15. Hapus department terpakai → 500.** `ER_ROW_IS_REFERENCED_2` tak
      ditangani errorMiddleware (hanya DUP_ENTRY + NO_REFERENCED_ROW).
      Fix: tangani → 409 dengan pesan jelas.
- [x] **B16. `my-assets` kosong → 404.** Seharusnya 200 `[]` (employee baru
      tanpa assignment tampil error, padahal state valid).
- [x] **B17. Update aset assigned.** `PUT /assets/:id` boleh ganti
      `asset_code/serial/category` saat `assigned/under_maintenance`.
      Fix: field identitas dibekukan saat aset sedang keluar/direparasi.
- [x] **B18. `roleMiddleware` crash tanpa auth.** Akses `req.user.role` sebelum
      cek `req.user` → `TypeError` 500 jika dipasang tanpa `authMiddleware`.
- [x] **B19. Token malformed → blank page.** `utils/auth.js` tanpa try/catch;
      `localStorage token="xxx"` → reload → exception.
- [x] **B20. Next aktif saat 0 hasil.** `total_pages=0`, `1!==0` → `setPage(0)`
      → fetch `page=0`. Footer kini tampil `0 / 0` + Next disabled.
- [x] **B21. Fetch tanpa Abort ×3.** Risk/history AssetDetail, 6 analytics
      Dashboard, Search aset CreateMaintenance → data basi menimpa.
      Fix: AbortController + cleanup (+ ref agar Search terbaru menang).
- [x] **B22. `department_id: 0` terkirim.** Tertutup B10: validasi tolak
      `department_id` kosong sebelum `Number("")` sempat dikirim.
- [x] **B23. Umur null → +30.** `getAgeScore(null)` = `new Date(null)` = 1970
      → aset tanpa tanggal selalu terlihat tua. Sekarang null → 0.
- [x] **B24. `canceled` hilang dari strip.** Ringkasan maintenance hanya hitung
      reported/in_progress/completed. Strip kini 4 kolom.
- [x] **B25. Tanpa route 404.** URL asing render kosong (hanya Toaster).
      Sekarang catch-all redirect ke `/`.
